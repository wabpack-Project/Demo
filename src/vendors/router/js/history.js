(function(root, factory) {
	...
	_History.prototype = {
		init: function (config) {
			...
			// 事件绑定设置
			this.bindWindowEvent(obj);

			// 判断路由模式
			// if (obj.mode === 'hash') {
			// 	// 事件绑定设置
			// 	this.bindHashWindowEvent();
			// } else {
			// 	// 事件绑定设置
		...
		// History事件绑定设置
		bindWindowEvent : function (obj) {
			// console.log("history事件绑定！");
			var _this = this;
			// 初始化跳转方法
			window.linkTo = function(path, tit) {
				console.log('path :', path);
				_this.hashChange(obj, path, tit);
			};

			//页面首次加载 匹配路由
			window.addEventListener('load', function(event) {
				console.log('load首次加载', event);
				_this.pageLoad(event, obj);
			}, false);

			// 根据路由模式绑定事件
			if (obj.mode === 'hash') {
				console.log("hash事件绑定！");
				//路由切换
				window.addEventListener('hashchange', function(event) {
					console.log('hashchange', event);
					_this.historyHashChange(event, _this.opts);
				}, false);
			} else {
				console.log("history事件绑定！");
				//路由切换
		...
		// 首次加载
		pageLoad : function(event, obj) {
			if (obj.mode !== 'hash') {
				this.historyChange(event, obj);
			} else {
				this.historyHashChange(event, obj);
			}
		},
		// 变更URL的HASH信息
		hashChange : function(obj, path, tit) {
			// 根据模式切换hash
			if (obj.mode !== 'hash') {
				if (path.indexOf("?") !== -1) {
					var _url = path + '&key=' + util.genKey();
				} else {
					var _url = path + '?key=' + util.genKey();
				}
				var state = {
		...
		// 路由历史纪录变化（hash）
		historyHashChange: function(event, obj) {
			var currentHash = util.getParamsUrl();		// 获取当前地址hash
			var nameStr = "router-" + (obj.routerViewId) + "-history";		// 拼接sessionStorage的key
			...
			window.sessionStorage[nameStr] = JSON.stringify(obj.history);
			// 根据路由跳转URL
			this.urlHashChange(obj, currentHash);
		},
		// 根据路由跳转URL
		urlChange: function(obj, currentHash) {
			...
			if (obj.routers[currentHash.path]) {
				...
				var state = {
					title : obj.routerMap[0].name,
					url		: location.href
				};
				currentHash = {
					path: obj.redirectRoute,
				};
				history.replaceState(state,null,'#' + currentHash.path);
				this.refresh(currentHash, obj);
			}
		},
		// 根据路由跳转URL
		urlHashChange: function(obj, currentHash) {
			// 判断当前hash的路由是否已经注册
			if (obj.routers[currentHash.path]) {
				// 刷新当前页面视图
				this.refresh(currentHash, obj);
